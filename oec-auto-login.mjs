...
import { createCDPClient, getOceanEngineTab, checkCDP } from './cdp-client.mjs';
import { sleep } from './wait-utils.mjs';
import { DATA_DIR, CAMPAIGN_URL, findLarkCli, FEISHU_CHAT_ID } from './monitor-utils.mjs';
...
export async function autoLogin(email, password) {
  ...
  if (!tab) {
    // 通过 /json/new 创建新标签
    try {
      const resp = await fetch('http://localhost:9222/json/new?' + encodeURIComponent('about:blank'));
      tab = await resp.json();
    } catch {}
  }
