...
- **监控 Dashboard（可选）**：独立隧道 `https://<dashboard-tunnel>/dashboard-v4?password=<pwd>` 或本地 `http://<局域网IP>:8899/dashboard-v4`；Dashboard 不经过 CodeBuddy 网关，避免网关的 `x-codebuddy-request` 校验拦截（地址拼接逻辑见 `send_gateway_to_feishu.py`）。
- **本地访问**：`http://192.168.1.5:8899/dashboard-v4`。
