  ...
  if (window3h) {
    ...
    if (window3h.burnRate.change > 0.6 && window3h.burnRate.second > 500) {
      ...
      alerts.push({
        type: 'burn_accel_3h',
        name: '消耗加速度异常',
        detail: `近1.5h燃烧速率 ¥${window3h.burnRate.second.toFixed(0)}/h，较前1.5h ¥${window3h.burnRate.first.toFixed(0)}/h 加速 ${accel}%`,
...
function generateHTML(analysis) {
  ...
  const campaignRows = (topNewSpenders || []).map(c => {
    ...
    <tr>
      ...
      <td>${c.conversions||0}</td>
      <td style="font-weight:bold;color:${c.cpa > summary.avgCPA * 1.3 ? '#e74c3c' : '#27ae60'}">¥${(c.cpa||0).toFixed(2)}${c.cpa15 > 0 ? `<br><span style="font-size:0.85em;color:#888">15m: ¥${c.cpa15.toFixed(2)}</span>` : ''}</td>
...
<div class="header">
  ...
  <div class="sub">更新时间: ${now} | 时段: ${d.timeSlot || 'N/A'} | 16h直播(7-23) | 有消耗 ${summary.totalSpending||0}条 · 投放中 ${summary.totalActive||0}条 · 起量 ${(rampingUp||[]).length} · 掉量 ${(dropping||[]).length} · 节奏 ${d.pacingHealth||'N/A'} | 冷启${d.lifecycle?.cold_start||0}·活跃${d.lifecycle?.active||0}·死亡${d.lifecycle?.dead||0} | 离线快照</div>
</div>
...
<div class="footer">
  WorkBuddy 自动监控 · ${today} · 巨量引擎 ${CONFIG.accountName} · 16h直播(7:00-23:00) · 环比T-15/30/60min · 离线快照
...
function buildFeishuCard(analysis) {
  ...
  if (topNewSpenders.length > 0) {
    ...
    for (let i = 0; i < Math.min(5, topNewSpenders.length); i++) {
      ...
      const cpl15Str = c.convDelta > 0 ? `¥${c.cpa15.toFixed(0)}` : '—';
      topLines.push(`${i + 1}. ${trendTag(c.trend)} ${c.name.slice(0, 18)} — ¥${c.spendDelta.toFixed(0)} (${rateStr}) · 15mCPL ${cpl15Str}`);
    }
  }
  ...
  // --- 底部按钮：HTML 报表已单独发送为文件 ---
  elements.push({ tag: 'hr' });
  elements.push({
    tag: 'note',
    elements: [
      { tag: 'plain_text', content: `📊 详实报表已发送为HTML文件，可在群聊中下载查看` }
    ]
  });
  ...
  elements.push({
    tag: 'note',
    elements: [
      { tag: 'plain_text', content: `🕐 ${now} · ${d.timeSlot || ''} · 16h直播(7-23) · 详实报表已发送文件 · 点击按钮反馈建议` }
    ]
  });
...
function sendReportFileToChat() {
  ...
  try {
    const larkCmd = CONFIG.larkCli.endsWith('.exe') ? CONFIG.larkCli
      : CONFIG.larkCli.endsWith('.ps1') ? CONFIG.larkCli.replace(/\.ps1$/, '.cmd')
      : CONFIG.larkCli.endsWith('.cmd') ? CONFIG.larkCli
      : CONFIG.larkCli + '.cmd';

    // lark-cli --file 要求相对路径(拒绝绝对路径和..)，cwd 设为报表目录
    const cwd = CONFIG.reportDir;
    const relFile = 'oceanengine-report.html';
    const result = spawnSync(larkCmd, [
      'im', '+messages-send',
      '--chat-id', CONFIG.feishuChatId,
      '--file', relFile,
    ], { timeout: 30000, encoding: 'utf-8', windowsHide: true, cwd });
    ...
    if (parsed.ok) {
      ...
      console.log(`  ⚠ 报表文件发送失败: ${parsed.error?.message || stdout}`);
      return false;
    }
  } catch (e) {
    console.log(`  ❌ 报表文件发送异常: ${e.message}`);
    return false;
  }
}
