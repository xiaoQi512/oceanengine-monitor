...
function analyzeData(campaigns, accountSpend = 0, accountBudget = 0, accountBalance = 0, pageSummary = null) {
  ...
  const avgCPM = active.length > 0 ? active.reduce((s, c) => s + c.cpm, 0) / active.length : 0;
  ...
  const prevCampaigns15 = prev.t15?.active?.length
    ? prev.t15.active
    : (prev.t15?.allSpending || []);
  const prevCampaigns30 = prev.t30?.active?.length
    ? prev.t30.active
  ...
  // 1. 消耗速度过快（当前15分钟消耗速度 vs 前1小时平均速度）
  if (speedHour > 0 && speedCurrent > speedHour * 1.5 && spendLast15min > 50) {
    const excess = ((speedCurrent / speedHour - 1) * 100).toFixed(0);
    alerts.push({
      type: 'speed',
      name: '消耗速度过快',
      detail: `近15分钟消耗 ¥${spendLast15min.toFixed(0)}，速度 ¥${speedCurrent.toFixed(1)}/min，较前1小时均速 ¥${speedHour.toFixed(1)}/min 涨幅 ${excess}%`,
      severity: speedCurrent > speedHour * 2 ? 'high' : 'medium',
    });
  }
  
  // 2. 成本上涨过快
  if (avgCPA > 0 && prevCPA30 > 0 && avgCPA > prevCPA30 * 1.2) {
    const cpaRise = ((avgCPA / prevCPA30 - 1) * 100).toFixed(0);
    const label = avgCPA > prevCPA30 * 1.5 ? 'high' : 'medium';
    alerts.push({
      type: 'cpa_rise',
      name: '成本上涨过快',
      detail: `当前平均CPL ¥${avgCPA.toFixed(2)}，较30分钟前 ¥${prevCPA30.toFixed(2)} 上涨 ${cpaRise}%`,
      severity: label,
    });
  }
  
  // 3. 日预算不足 + 节奏异常
  // effectiveBudget 已在上方（基础汇总区）计算
  ...
  if (pacingHealth === 'danger' && pacingRatio > 1.5 && budgetUsed < 0.85) {
    alerts.push({
      type: 'pacing_fast',
      name: '消耗节奏过快',
      detail: `已消耗 ¥${totalSpend.toFixed(0)} (${(budgetUsed*100).toFixed(0)}%)，远超时间进度 ${(timeProgress*100).toFixed(0)}%，按此节奏预估将超预算`,
  ...
  if (pacingHealth === 'danger' && pacingRatio < 0.6 && timeProgress > 0.3) {
    alerts.push({
      type: 'pacing_slow',
      name: '消耗进度严重落后',
      detail: `已消耗 ¥${totalSpend.toFixed(0)} (${(budgetUsed*100).toFixed(0)}%)，落后时间进度 ${(timeProgress*100).toFixed(0)}%，建议检查计划投放状态`,
      severity: 'medium',
    });
  }
...
function buildFeishuCard(analysis) {
  ...
  const watchAlerts = alerts.filter(a => a.type === 'cpa_rise' || a.type === 'dropping' || a.type === 'cpa_trend' || a.type === 'spend_trend' || a.type === 'account_budget_cap');
  const infoAlerts = alerts.filter(a => a.type === 'speed' || a.type === 'budget' || a.type === 'pacing_fast' || a.type === 'pacing_slow' || a.type === 'dead_plan');
  ...
  if (watchAlerts.length > 0) {
    ...
    for (const a of watchAlerts) {
      alertLines.push(`${a.type === 'cpa_rise' ? '📈' : a.type === 'account_budget_cap' ? '🏦' : '📉'} ${a.name}: ${a.detail}`);
    }
  }
