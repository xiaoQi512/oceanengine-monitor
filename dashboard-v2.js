...
function dashV2() {
  return {
    ...
    // === 分组数据（对应 /api/campaigns/grouped 返回结构） ===
    groups: {
    ...
    expandedGroups: { '简单投': true, '画面直投': true, '短引直': false },  // 默认展开前两组
    ...
    get spendingPlanCount() {
      const last = this.trendData.spendingCount;
    ...
    async loadData() {
      ...
      try {
        ...
        // [1] /api/snapshots/5m -> 最新快照（覆盖 live-status 的默认 KPI）
        // 仅当 live-status 的 KPI 为默认值 0 且快照有真实数据时才覆盖
        if (results[1].status === 'fulfilled' && results[1].value && results[1].value.latest) {
          const s = results[1].value.latest;
          if (s.accountSpend !== undefined && s.accountSpend > 0 && this.kpi.totalSpend === 0) this.kpi.totalSpend = s.accountSpend;
          if (s.accountBudget !== undefined && s.accountBudget > 0 && this.kpi.dailyBudget === 45000) this.kpi.dailyBudget = s.accountBudget;
          if (s.totalConv !== undefined && s.totalConv > 0 && this.kpi.totalLeads === 0) {
            this.kpi.totalLeads = s.totalConv;
            this.kpi.totalConversions = s.totalConv;
          }
          if (s.accountSpend > 0 && s.totalConv > 0 && this.kpi.avgCpl === 0) {
            this.kpi.avgCpl = Number((s.accountSpend / s.totalConv).toFixed(2));
          }
          // [v1.5] 消耗速度数据
          if (s._rolling) {
        ...
        // [3] /api/campaigns/grouped -> 分组数据
        if (results[3].status === 'fulfilled' && results[3].value) {
          const d = results[3].value;
          if (d.groups) {
            // 兜底处理 null 值（API 可能返回 spend: null）
            for (const gname of Object.keys(this.groups)) {
              const g = d.groups[gname];
              if (g) {
                this.groups[gname] = {
                  summary: g.summary || emptyGroupSummary(gname),
                  plans: (g.plans || []).map(p => this._normalizePlan(p)),
                };
              }
            }
          }
          this.ungrouped = (d.ungrouped || []).map(p => this._normalizePlan(p));
      ...
      _trendChart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            ...
            // [Y轴3 - 数量] 在投计划柱状（浅色背景柱：campaigns.status='投放中'）
            ...
            // [Y轴3 - 数量] 消耗中计划柱状（深色前景柱：cost > 0）
            { label: '消耗中计划', data: this.trendData.spendingCount || [], type: 'bar', backgroundColor: 'rgba(6,182,212,0.35)', borderColor: '#06b6d4', yAxisID: 'y2', borderWidth: 1.5, barPercentage: 0.6, categoryPercentage: 0.7 },
          ],
        },
        options: {
          responsive: true,
            ...
            tooltip: {
              ...
              callbacks: {
                ...
                label: (ctx) => {
                  ...
                  if (ds.label === '消耗中计划') { return '消耗中计划: ' + v + ' 个'; }
                  return '';
                },
                afterBody: (ctx) => {
                  const i = ctx[0].dataIndex;
    ...
    updateTrendChart(newData) {
      ...
      let displaySpendingCount = newData.spendingCount || [];
      if (this.trendMode === 'delta') {
        // 5分钟增量：消耗、转化做差分
        displaySpend = newData.spend.map((v, i) => i === 0 ? 0 : Number((v - newData.spend[i - 1]).toFixed(2)));
        displayConv = newData.conversions.map((v, i) => i === 0 ? 0 : v - newData.conversions[i - 1]);
        // 增量 CPL：deltaSpend / deltaConversions
        const deltaCplArr = [];
        for (let i = 0; i < displaySpend.length; i++) {
          deltaCplArr.push(i === 0 || displayConv[i] <= 0 ? 0 : Number((displaySpend[i] / displayConv[i]).toFixed(2)));
        }
        _trendChart.data.datasets[1].data = deltaCplArr;  // 5min CPL
        // 增量 CPM：deltaSpend / deltaImpressions * 1000
        const deltaCpmArr = [];
        const imprArr = newData.impressions || [];
        for (let i = 0; i < displaySpend.length; i++) {
          if (i === 0 || !imprArr[i] || !imprArr[i - 1]) { deltaCpmArr.push(0); continue; }
          const deltaImpr = imprArr[i] - imprArr[i - 1];
          deltaCpmArr.push(deltaImpr > 0 ? Number(((displaySpend[i] / deltaImpr) * 1000).toFixed(2)) : 0);
        }
        ...
        displaySpendingCount = displaySpendingCount.map(v => v ?? null);
      } else {
        // 累计模式：用后端返回的原始数据
      ...
      _trendChart.data.datasets[5].data = displaySpendingCount;
    ...
    // 单条推送补推：按 pushType 发送信号
    async repushEntry(entry) {
      this.toastShow('正在补推 ' + (entry.type || '') + '...', 'info');
      try {
        ...
        const d = await r.json();
        if (d.ok) this.toastShow('补推信号已发送 (' + (entry.type || '') + ')', 'success');
