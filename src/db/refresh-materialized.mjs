...
export const REFRESH_LOCK_FILE = path.join(PROJECT_ROOT, 'monitor-data', 'refresh-materialized.lock');
export const STALE_REFRESH_TIMEOUT_MS = 5 * 60 * 1000;
...
export function readRefreshLockInfo() {
  if (!fs.existsSync(REFRESH_LOCK_FILE)) return { pid: null, time: null, raw: null };
  try {
    const raw = JSON.parse(fs.readFileSync(REFRESH_LOCK_FILE, 'utf-8'));
    const pid = Number.isInteger(raw.pid) && raw.pid > 0 ? raw.pid : null;
    const time = typeof raw.time === 'string' ? raw.time : null;
    return { pid, time, raw };
  } catch {
    return { pid: null, time: null, raw: null };
  }
}
...
export function acquireRefreshLock() {
  ...
  if (!fs.existsSync(REFRESH_LOCK_FILE)) {
    fs.writeFileSync(REFRESH_LOCK_FILE, JSON.stringify({
      pid: process.pid,
  ...
  let lockAgeMs = NaN;
  try {
    lockAgeMs = Date.now() - fs.statSync(REFRESH_LOCK_FILE).mtimeMs;
  } catch {}
  const stale = !info.pid
    || !isPidAlive(info.pid)
    || !Number.isFinite(lockAgeMs)
    || lockAgeMs > STALE_REFRESH_TIMEOUT_MS;
...
export function refreshMaterialized() {
  ...
  try {
    ...
    if (acquired) releaseRefreshLock();
  }
}
