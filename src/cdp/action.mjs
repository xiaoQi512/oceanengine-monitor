...
import { quickConnectWithRetry } from './client.mjs';
import { extractCookiesFromBrowser } from './cookie-provider.mjs';
import { waitForCondition } from '../utils/wait-utils.mjs';
import { createClient as createApiClient, getProjects as apiGetProjects } from '../platform/oec-client.mjs';
import { DATA_DIR } from '../config/index.mjs';

const SCREENSHOT_DIR = path.join(DATA_DIR, 'action-screenshots');
const AUDIT_LOG_FILE = path.join(DATA_DIR, 'action-audit.jsonl');
...
async function readPlanStateFromApi(planName) {
  try {
    const apiClient = await createApiClient({ useCache: true, cookieExtractor: extractCookiesFromBrowser });
