...
import { DATA_DIR, ACCOUNT_ID } from '../config/index.mjs';

// ====== 常量 ======
const COOKIE_CACHE_FILE = path.join(DATA_DIR, '.oec-cookies.json');
const BASE_URL = 'https://ad.oceanengine.com';
export const OEC_COOKIE_CACHE_FILE = COOKIE_CACHE_FILE;
export const OEC_BASE_URL = BASE_URL;

// ====== 名称覆盖缓存 ======
let _nameOverridesCache = null;
let _nameOverridesCacheTime = 0;
const NAME_OVERRIDE_CACHE_TTL = 5 * 60 * 1000; // 5分钟缓存

function loadNameOverrides() {
  const now = Date.now();
  if (_nameOverridesCache && (now - _nameOverridesCacheTime) < NAME_OVERRIDE_CACHE_TTL) {
    return _nameOverridesCache;
  }
  try {
    const dbPath = path.join(DATA_DIR, 'oceanengine.db');
    if (!fs.existsSync(dbPath)) { _nameOverridesCache = []; return []; }
    const db = new Database(dbPath, { readonly: true });
    const rows = db.prepare('SELECT name_pattern, replacement FROM name_overrides').all();
    db.close();
    _nameOverridesCache = rows || [];
    _nameOverridesCacheTime = now;
  } catch {
    _nameOverridesCache = [];
  }
  return _nameOverridesCache;
...
function applyNameOverrides(name) {
...
// ====== Cookie 提取（通过依赖注入接入 CDP，保持 platform 只依赖 utils/config） ======
let defaultCookieExtractor = null;

export function setCookieExtractor(extractor) {
  defaultCookieExtractor = extractor;
}

async function extractCookiesFromBrowser() {
  if (!defaultCookieExtractor) {
    throw new Error('浏览器 Cookie 提取器未配置，请通过 src/services/api-client.mjs 或 cookieExtractor 注入');
  }
  return defaultCookieExtractor();
}
...
export async function collectAllData(client) {
  ...
  const campaigns = allProjects.map(p => {
    ...
    return {
      id: p.project_id || '',
      name: applyNameOverrides(p.project_name || ''),
      status,
      rawStatus: apiStatus,
...
export async function createClient(options = {}) {
  const { forceRefresh = false, useCache = true, cookieExtractor } = options;
  const extractCookies = cookieExtractor || defaultCookieExtractor;
  let cookieData = null;
  ...
  if (!cookieData) {
    if (!extractCookies) throw new Error('Cookie 提取器未配置，且没有可用缓存');
    cookieData = await extractCookies();
  }

  return {
    cookieData,
    refreshCookies: () => extractCookies ? extractCookies() : extractCookiesFromBrowser(),
