...
import { CHROME_USER_DATA_DIR, CHROME_PROFILE_DIRECTORY, findChromeExe } from './monitor-utils.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const LOG_FILE = path.join(__dirname, 'monitor-data', 'chrome-guard.log');
...
async function main() {
  ...
  log(`   Chrome: localhost:9222`);
  log(`   CDP proxy: localhost:3456`);

  let consecutiveFailures = 0;

  while (true) {
    try {
      const chromeOk = await probe('http://localhost:9222/json/version');
      const proxyOk = await probe('http://localhost:3456/targets');
