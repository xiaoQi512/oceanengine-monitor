...
export async function getShiftDelta(dateStr, shift, options = {}) {
  ...
  const { createClient: _createClient } = await import('../oceanengine-api-client.mjs');
  const client = apiClient || await _createClient({ useCache: true });
  ...
  const { getSessionStats } = await import('../oceanengine-api-client.mjs');
