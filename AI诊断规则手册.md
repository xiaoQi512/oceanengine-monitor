...
| v1.2 | 2026-08-15 | 低耗停滞预警（Rule-002） | 每15分钟 | 自动告警（alerts） |
...
### Rule-002：低耗停滞预警（投放中但时均消耗 < ¥50）

**目标**：自动识别"投放中但持续抢不到量"的计划，生成预警，供人工判断是否提价或暂停。

**触发条件**（`lifecycle-analysis.mjs` 的 `_lifecycle === 'dead'`）：
- 计划状态为投放中（active）
- 已投放时长 ≥ 3 小时
- 时均消耗 < ¥50（`总消耗 ÷ 已投放小时数 < 50`）

**输出**（`plan-alerts-trend.mjs` 的 `buildDeadPlanAlerts`）：
- 告警类型 `dead_plan`
- 名称：`N 条计划低耗停滞`
- 明细：各计划「时均消耗 < ¥50 且已投放 ≥ 3h」
- 严重度：low（自动推送至飞书卡片 alertLines）

**数据来源**：15 分钟快照 `active` 数组（`computeLifecycleFromSnapshots` 基于近 6 小时快照序列推断）。

**人工处置建议**（规则仅标注，不自动操作）：
- 竞价失败抢不到量 → 提高出价（+5% / +5 元）
- 素材衰退或计划老化 → 暂停止损

**更新频率**：每 15 分钟随监控脚本自动执行。

**维护记录**：2026-08-15 阈值由 ¥100 调整为 ¥50（用户明确"每小时消耗 < 50"口径）。
