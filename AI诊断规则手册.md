...
> 用途：为 AI 提供结构化诊断规则，用于分析 15 分钟快照数据并生成运营建议
> 原则：每条规则独立可校验，包含数据来源、判断条件、输出格式、更新频率
...
| 版本 | 日期 | 规则 | 更新频率 |
|------|------|------|:---:|
| v1 | 2026-07-28 | 直播间消耗节奏基线（Rule-001） | 每周一次 |
...
| 节奏级别 | 条件 | 标签 | 建议 |
|----------|------|------|------|
| 过慢 | 当前时均速度 < 15天均值 - 2×stdev | pacing_slow | 检查是否有计划因预算不足停止投放；考虑开启新计划 |
| 偏慢 | 当前时均速度 < 15天均值 - 1×stdev | pacing_slightly_slow | 关注趋势，暂不操作 |
| 正常 | 15天均值 ± 1×stdev 范围内 | pacing_normal | 正常投放 |
| 偏快 | 当前时均速度 > 15天均值 + 1×stdev | pacing_slightly_fast | 关注是否预算透支，检查 CPL |
| 过快 | 当前时均速度 > 15天均值 + 2×stdev | pacing_fast | 检查 CPL 是否飙升；考虑暂停部分低效计划 |
...
| 节奏 | CPL | 综合判断 |
|------|-----|--------|
| pacing_fast | CPL < 15天均值 | 高效放量，无需干预 |
| pacing_fast | CPL > 15天均值 + 1×stdev | CPL 恶化，建议暂停低效计划 |
| pacing_slow | CPL < 15天均值 | 高效但消耗不足，检查预算/计划数 |
| pacing_slow | CPL > 15天均值 | 双低，需要排查计划健康度 |
...
3. 联动 `summary.avgCPA` 和 `_multiDay.cpa.mean` 做 CPL 校验
4. 给出合并诊断：节奏 + CPL 联动 + 可操作建议
